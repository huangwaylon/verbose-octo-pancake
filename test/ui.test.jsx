 ...
 * a control opens in, which ARIA wiring resolves, what a fallback picks. A case belongs here if
 * a correct-looking screen could still be wrong, in `render.test.jsx` if it would look broken.
 */
...
/**
 * Two steps: the attributes have to sit on the PANEL — as three separate substring checks this
 * passes with them moved onto the backdrop's wrapper — and the name has to RESOLVE.
 */
...
describe('foldTail', () => {
  ...
  it('keeps exactly MAX_SLICES without folding', () => {
    ...
    // The whole list back, identical. A length assertion cannot see the off-by-one it is
    // named after: folding a 6-item list yields 5 kept plus "Other", which is still 6 long.
    expect(foldTail(items, 'Other')).toEqual(items)
  })
...
describe('DonutChart', () => {
  ...
  it('assigns the validated series slots in fixed order, never cycling', () => {
    ...
    // Must be an inline STYLE: var() is invalid in an SVG presentation attribute, and a CSS
    // rule on .chart__slice paints every slice one colour — a chart that passes every test.
...
describe('entry form', () => {
  ...
  it('orders the fields by how often each is touched', () => {
    // Amount and note are typed every time; the rest default to something usually right, the
    // category to a guess. Reading order is the design of this form, and nothing else sees it.
    ...
    // Sorted by where each appears, so a failure names the fields that swapped.
  ...
  it('takes the whole screen for an expense but not for a settlement', () => {
    // Full screen is a claim about the CONTENT: a settlement is three fields.
  ...
  it('offers each configured note as a datalist option and as a chip', () => {
    ...
    for (const shop of ['OK Mart', 'Ozeki']) {
      // The chip specifically: a bare `toContain(shop)` passes on the datalist `<option>`
      // alone, and the chip row exists because a datalist has no affordance on a phone.
  ...
  it('keeps a stored category the config tab no longer lists, and selects it', () => {
    // A `<select>` whose value matches no option renders blank and then silently saves the
    // invisible old value, quietly rewriting a row whose category was renamed elsewhere.
  ...
  it('applies each payer’s own default, so 80/20 does not invert when p2 pays', () => {
    // The whole point of the per-person setting: p1 bears 80% of what p1 paid, p2 20% of p2's.
  ...
  it('states who owes what, in whole yen that add back up to the amount', () => {
    // 4211 is odd on purpose: the payer's half rounds half-up and the other takes what is
    // left, so 2105.5 printed twice — inventing or losing a yen on screen — cannot pass.
  ...
  it('states nothing until there is an amount to divide', () => {
    // "¥0 · ¥0" would be a claim about money nobody has typed.
    expect(render({}, { amountYen: 0 })).not.toContain('¥0')
  })

  it('drops the note, the category AND the split from a settlement, and says why', () => {
    // The three sit either side of the payer and date controls, so the form carries two
    // `!isSettlement` blocks and a field left in the wrong one is invisible unless all three
    // are named. `type="range"` is no test of the split: an even split renders no slider.
    const markup = render(
      { notePresets: ['OK Mart'] },
    ...
    // The payer control stays, and its hint is what explains the row.
  ...
  it('offers a plain numeric keypad, because the yen has no sub-unit', () => {
    // A decimal point on the pad would invite an amount 100x wrong.
    const markup = render()
...
// A different DECISION from the entry form's order: a template is filled in once and then read
// in a list, so the name leads because it is the only thing naming the row on screen.
describe('recurring form', () => {
  ...
  it('states who owes what, from the template’s own amount and share', () => {
    // The same control and derivation the entry form uses: a second one is how a template's
    // figures and an entry's come to disagree.
  ...
  // Default mode saves a BLANK share, so there is nothing to divide by. Printing the config
  // tab's split would read as the figures this cost saves, which is what it does not do.
  ...
  it('always offers the stored category, even when the config tab dropped it', () => {
    // The `CategoryField` money bug: a `<select>` matching no option saves the invisible value.
...
// Cancel first in the DOM is the rule nothing else can see: `BottomSheet` focuses the first
// control it finds, which on a destructive dialog has to be the way out.
describe('confirm dialog', () => {
  ...
  it('is content-sized, never full screen', () => {
    // `full` is a claim about the CONTENT: one sentence in a full-screen panel is 600px of white.
...
describe('deleted entries list', () => {
  ...
  // Without the icon, the class and the direction it read as an expense the payer had bought
  // something with — the one row where losing the direction reverses what Restore will do.
  it('keeps a settlement recognisable as one after it is deleted', () => {
    ...
    // Both names, in order — never just "You paid", which is what an expense row says.
    expect(markup).toContain('You paid Sam')
  })
})

describe('Japanese rendering', () => {
  // One string per component is under test: the balance direction needs a debtor, the summary
  // both people's spend. The 800 is `payerShare: 1` so the two are not mirror images.
  const entries = [
  ...
  it('renders the whole app surface in Japanese, differently from English', () => {
    ...
    // One string from each of the three components, so dropping any of them fails here.

...
import { defaultSplitFor, nextSplit, toSplit } from '../src/lib/split.js'
...
describe('toSplit', () => {
  it('shows an even share as Even, with the slider hidden', () => {
    expect(toSplit(EVEN_SHARE)).toEqual({ mode: 'even', percent: 50 })
  })

  it('opens Custom on the stored number, so a configured 80% lands ready to submit', () => {
    expect(toSplit(0.8)).toEqual({ mode: 'custom', percent: 80 })
    expect(toSplit(0)).toEqual({ mode: 'custom', percent: 0 })
    expect(toSplit(1)).toEqual({ mode: 'custom', percent: 100 })
  })
})
...
describe('nextSplit', () => {
  it('returns to the payer’s configured default when Custom is re-opened', () => {
    ...
    expect(nextSplit('custom', 0.8)).toEqual({ mode: 'custom', percent: 80 })
    expect(nextSplit('custom', 0.2)).toEqual({ mode: 'custom', percent: 20 })
  })

  it('is exactly even when Even is chosen, whatever the default is', () => {
    expect(nextSplit('even', 0.8)).toEqual({ mode: 'even', percent: 50 })
    expect(nextSplit('even', EVEN_SHARE)).toEqual({ mode: 'even', percent: 50 })
  })

...
/**
 * The one piece of arithmetic behind the sheet clearing the software keyboard. `position:
 * fixed` and `dvh` both anchor to the layout viewport, which iOS does not shrink for the
 * keyboard, so this difference is the only signal there is — and getting it wrong leaves Save
 * under a keypad that has no Done key.
 */
describe('keyboardInset', () => {
  ...
  it('accounts for the visual viewport being scrolled within the layout one', () => {
    // iOS shifts the visual viewport to follow the focused field, so ignoring offsetTop
    // reports an inset smaller than the keyboard and the footer stays half-covered.
  ...
  it('reads a zoomed viewport at its unzoomed scale, rather than inventing a keyboard', () => {
    // Zoom shrinks the visual viewport exactly as a keyboard does, so the bare difference
    // reports 426px of keypad that is not there.
    expect(keyboardInset({ innerHeight: 852, height: 426, offsetTop: 0, scale: 2 })).toBe(0)
    // A keyboard UNDER that zoom is still real: at 2x a 336px keypad covers 168 CSS px.

 ...
 * `batchGet`, and the sheet read before fresh data — so every millisecond spent getting
 * here is added to the wait in full. Asking from an effect means waiting for the whole
 * first render to commit, which on a cached launch is a paint of the entire ledger and
 * grows with it: measured at 90ms behind a 120-entry snapshot and 165ms behind 400, on a
 * 4x-throttled CPU.
 *
 * It costs nothing when there is nothing to do: no key, no request. `tokenAtLeast` shares
 * the single in-flight mint, so `useConnection`'s bootstrap and the ledger's first read
 * join THIS one. The rejection is swallowed because both of those report the failure
 * themselves, with a retry attached.
 *
 * Prod-only for the same reason as the service worker below: nothing imports `main.jsx`,
 * but the rule that these modules must not reach the network at import time under vitest
 * is worth keeping visibly true.
 */

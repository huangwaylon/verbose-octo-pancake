...
| `src/lib/{snapshot,launchRead,serviceWorker,viewport,preference}.js` | the launch cache: last successful read, kept on the device; the first sheet read, started before React renders; registration, and when it is safe to activate an update; how much of the layout viewport the keyboard covers; the per-device store the locale, the accent and the summary view share; which person this device is, ISO date helpers, accent presets, the formatter cache (`{identity,dates,theme,memo}.js`) |
| `src/state/`, `src/components/`, `src/i18n/`, `src/styles/` | `useConnection`, `useLedger` (optimistic CRUD, throttled focus refresh), `useLedgerView` (every derived figure), `useToasts`, `useKeyboardInset`; `LedgerScreen.jsx` is the whole signed-in surface, with `App`, the visual harness and one render test its three callers; one file per view, with inline-SVG icons and chart, and `useEntrySplit`/`useSheetSave` beside the control holding their state; the i18n engine and `en`/`ja` catalogs; `tokens`/`base`/`primitives`/`app` in that order |

...
- **An id is unique in neither tab.** Every read goes through `reconcileById`, every write to an existing row through `resolveRow`, and **the two must choose the same row** — a write to a copy nobody can see is an edit that reverts and a delete that undoes itself. Both rank live over dead, then the FIRST live row, then the LATEST `deleted_at`; one fixture in `test/sheets.test.js` drives both. What `reconcileById` hid is counted twice over: tombstones as `supersededRows`, for `compact`, and live rows as `duplicateRows`, which only the sheet can fix. Every failure here is silent.
- **`updateEntry`/`setDeletedAt` must be told the row's CURRENT payer** — `previous.payer`, never `entry.payer`; both throw `error.entryGone` for an entry that has left state, and `updateEntry` throws on a `previousPayer` that is not a person. That is why `removeEntry`/`restoreEntry` take an id alone.
...
- **`compact()` and `deleteTemplate` are the only hard deletes.** `compact` reads full rows rather than the `deleted_at` column, serializes its tab reads, and deletes in **descending** row order — all three protect row numbers derived from position. It takes a row only when the id column AND `deleted_at` are filled, because a stamp with no id is a row in the wrong columns, which `loadAll` counts and asks a person to fix. It covers `DATA_TABS`, never `RECURRING`.
- **`assertGid` guards every request carrying one**: `JSON.stringify` drops an undefined `sheetId` and a request without one acts on gid 0, so a lost gid deletes from — or appends to — the first tab. `missingGid` is the shared upstream check all three callers make, and `useLedger` throws rather than let a skipped tab pass unsaid. **`compact` never runs while a write is in flight, and reports `busy` rather than `{removed: 0}`.**
...
- **`blocksReload` decides whether an update may reload, and it is in `lib/`.** Three inputs because `pending` covers none of the last: an open form, an unacknowledged write, and `useLedger`'s `writing` count. Never SOLVE it by adding overlay kinds or a second "which sheet is open" value.
...
- **A hook holds effects; the decisions live in `lib/`.** With no DOM and no hook renderer in the tests, nothing a `use*.js` decides for itself is reachable — see `ledgerState.js`, `balance.js`, `split.js`, `recurring.js`. `useEntrySplit` and `useSheetSave` each sit beside the one control holding their state.

## Money, dates and the split
- **Yen only, and no function takes a currency** — an amount IS an integer number of yen, and re-adding the argument without the column is the one shape that IS a silent 100x error. `JPY` is spelled once, in `money.js`'s `Intl` call.
...
- **Dates are ISO strings, compared as strings** — never `new Date('2026-08-05')`, which is UTC midnight. `lib/dates.js` owns every helper and both shape checks: `isIsoDate` needs its UTC round-trip, since the regex alone accepts `2026-02-31`. `monthParts` checks SHAPE before numbers, and **`dayInMonth` CLAMPS**.
- **The pure layers stay pure**: `money`, `dates`, `balance`, `schema`, `split`, `identity` never read the i18n singleton and never call argless `localeCompare`; locale arrives as an argument with an English default, and display formatters are cached on everything that decides one.
...
- **A settlement's payer is case-folded on read, and the FIRST usable value per key wins.** The category `<select>` always offers the entry's stored category, or it renders blank and saves the invisible old value.
...
- **Anything the sheet holds and the app cannot show is counted and said out loud** — `loadAll` returns six counts and `noticeKeys` owns their precedence, worst first. A row with nothing in its id column counts: that is what a row shifted out of column A looks like, and it was the one shape nothing said a word about. Never repair `configMissing`: re-seeding writes this build's defaults into an unknown sheet.
...
- **The cache is written from the screen, only once nothing is pending**, and it stores the PRE-MERGE config; `clearSnapshot` resets the remembered payload, and a successful read seeds it.
- **`setSafeToReload` must stay wired, and `reconsiderUpdate` with it** — without the nudge a worker refused while a form was open is never asked again. Its one-hour floor is the same `shouldRefresh` the sheet read uses.
...
- **Prettier owns formatting, `.js`/`.jsx` only** — stylesheets and docs are hand-tuned and outside the glob. For a literal that is a table use `// prettier-ignore`, whose text must be exactly that or it is silently inert; never widen `.prettierignore`, which needs `dist/` alone.
- **No new npm dependencies** without a clear reason; icons are inline SVG. One is also a CSP decision, and **adding a Google host means updating the CSP** in `index.html`. **Never put a real secret in a `VITE_` variable**: Vite inlines every one into the bundle, which is why `VITE_BASE` is read from `process.env` in `base.js` instead.
...
- **One helper, one home.** `readStored`/`writeStored` are the only `localStorage` touches; `storedPreference` the only per-device store; `cellText` in `schema.js`; every date helper in `lib/dates.js`; `PEOPLE` the only `[p1, p2]` literal; `UNCATEGORIZED` the only spelling of it; `useEntryTitle` the only place an entry becomes a title and `templateTitle` a template's, which is pure because `App`'s confirmation cannot call a hook; `isSettlement` in `schema.js`; `percentOf` in `split.js`, the one rounding of a share for display; `usePeopleLabels` the only place a component names a person, in three forms — `possessive` exists because English inflects.
- **A control that appears twice is a component**: `Field`, `Segmented`, `EntryLine`, `NoteField`, `SplitField`, `CategoryField`, `AmountField`, `PayerField`, `SheetFormFooter`, `OpenSheetLink`, `BottomSheet`, `ConfirmSheet`. `Field` is the one home of the `<label htmlFor>` vs `<span>` decision and of the "(optional)" marker, and the two radio groups — `Segmented` and the accent swatches — each carry their own `role="radiogroup"`. A wrapper with no job of its own is not on the list; inline it.
...
- **Two keys whose text is identical AND adjacent on screen are one key** — a `Field` label above a button repeating it verbatim — and a Japanese label collapsing to a bare verb its English twin qualifies is a bug parity tests cannot see. **A test that calls `setLocale` must restore it.**
...
- **Nothing may scroll sideways at 320px.** `.sheet__body` sets `overflow-x: hidden` explicitly; anything holding config-tab text needs `min-width: 0` and `overflow-wrap: anywhere` — not `break-word`, which does not reduce min-content width — including outside a sheet, where nothing clips it and the whole PAGE scrolls. Both `.layout` tracks carry the guard. The `preview-en-stress*` pages are the check.
- **The toast stack takes no pointer events**, or it swallows a tap on a delete control.
- **A row is a row, not text**: `button.entry__main` suppresses selection and the callout, and stays ELEMENT-qualified because the deleted list renders the same class on a `<span>` of inert text.
...
- **The `--series-N` order is the colorblindness-safety mechanism, not cosmetic** — a validated 6-slot categorical set including the ring's wrap-around pair. Never reorder, never cycle past slot 6; a 7th folds into "Other", and an accent preset never touches them. **Two values is not a pie**: the who-paid split is a meter bar with a hairline.
...
- **Use the tokens** — `var(--transition-*)` rather than a duration, which is what collapses under `prefers-reduced-motion`. The two COLOUR tokens copied into `index.html` and the manifest are pinned to `tokens.css`.
...
- **`--header-height` must never understate the header's height** — `.layout__aside` reads it from outside, so a short token slides the aside under the band, and `min-height` has to be the binding constraint.
...
- **Every gid is read fresh, every time, and never through `ensureStructure`** — `compact`, `deleteTemplate` and now every APPEND, which is why adding an expense is two calls and a payer move four. `values.batchGet` cannot reveal a gid, and `ensureStructure` WRITES: on a ledger whose config tab was deleted it re-seeds this build's defaults and takes the `configMissing` notice with them. There is deliberately no cached gid state to make that shortcut look free.
- **An endpoint that dies about a week after setup is the consent screen**, not a quota problem — and **nothing detects which person this is**: `IdentityGate` and the `localStorage` choice behind it are the only path, not a fallback.

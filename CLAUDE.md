...
- **`deleteRowRequest` asserts its gid**: `JSON.stringify` drops an undefined `sheetId` and a `GridRange` without one means gid 0, so a lost gid silently deletes from the first tab. `missingGid` is the shared upstream check both callers make, and `useLedger` throws rather than let a skipped tab pass unsaid. **`compact` never runs while a write is in flight, and reports `busy` rather than `{removed: 0}`.**
...
- **A control that appears twice is a component**: `Field`, `Segmented`, `EntryLine`, `NoteField`, `SplitField`, `CategoryField`, `AmountField`, `SheetFormFooter`, `OpenSheetLink`, `BottomSheet`, `ConfirmSheet`. `Field` is the one home of the `<label htmlFor>` vs `<span>` decision, and the two radio groups — `Segmented` and the accent swatches — each carry their own `role="radiogroup"`. A wrapper with no job of its own is not on the list; inline it.
